...
std::vector <INT>vaHd_rr;
std::vector <INT>vaTl_rr;
...
void validHead(REAL *con) {
    ...
    for (INT j = 0; j < entityTotal; j++) {
        if (j != h) {
            REAL value = con[j];
            if (value < minimal && ! _find(j, t, r)) l_filter_s += 1;
        }
    }
...
void validTail(REAL *con) {
    ...
    for (INT j = 0; j < entityTotal; j++) {
        if (j != t) {
            REAL value = con[j];
            if (value < minimal && ! _find(h, j, r)) r_filter_s += 1;
        }
    }
...
extern "C"
REAL getValidHit10() {
    return (l_valid_filter_tot / validTotal + r_valid_filter_tot / validTotal) / 2;;
}

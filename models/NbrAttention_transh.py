...
class NbrAttention_transh(Model):
	...
	def embedding_def(self):
		...
		self.rel_norm_embeddings = tf.get_variable(name='rel_norm_embeddings', shape=[(config.relTotal+1), config.sem_dim], initializer=tf.contrib.layers.xavier_initializer(uniform=False)) #norm vectors of planes
		...
		self.parameter_lists = {"ent_c_embeddings":self.ent_c_embeddings, "ent_sm_embeddings":self.ent_sm_embeddings, "rel_norm_embeddings":self.rel_norm_embeddings, 
	...
	def _aggregation(self, nbr_embeddings, input_embeddings, nbrs, hd_max):
		...
		mask = tf.nn.embedding_lookup(mask_emb, tf.reshape(nbrs, [-1, hd_max])) 
		pos_dm_nbrs_vec = tf.nn.embedding_lookup(nbr_embeddings, tf.reshape(nbrs, [-1, hd_max])) 
		pos_dm_nbrs_vec = pos_dm_nbrs_vec * mask 
		...
		pos_cal_att_final = tf.nn.softmax(pos_cal_att) 
	...
	def _irr_new(self, pos_emb_h, pos_emb_t, pos_emb_r_sm, pos_rel_ss, pos_rel_oo): #(bz,1),...,(bz,1,type_dim)
		pos_h_t_bool = tf.cast(tf.equal(pos_emb_h, pos_emb_t), dtype=tf.float32) 
		pos_h_t_bool_rev = tf.ones_like(pos_h_t_bool)-pos_h_t_bool 
		pos_irr_rel = tf.nn.sigmoid(tf.reduce_sum(tf.concat([pos_emb_r_sm, abs(pos_rel_ss-pos_rel_oo)], axis=-1) * self.irr_w, axis=2)) 
		pos_irr_type = tf.nn.sigmoid(tf.reduce_sum(pos_rel_ss*pos_rel_oo, -1)) 
	...
	def _asym_new(self, pos_rel_ss, pos_rel_oo, pos_emb_h_sm, pos_emb_r_sm, pos_emb_t_sm): #(bz,1,dim)
		...
		pos_asym_pred_m = tf.nn.sigmoid(tf.reduce_sum(tf.concat([pos_emb_r_sm], axis=-1) * self.asym_w, axis=2)) #(bz,1,dim)->(bz,1)
	...
	def loss_def(self):
		...
		pos_emb_h, pos_emb_t, pos_emb_r, pos_y, pos_dm_nbrs, pos_dm_nbrs_len, \
		pos_rg_nbrs, pos_rg_nbrs_len = self.get_positive_instance(in_batch = True)
		
		neg_emb_h, neg_emb_t, neg_emb_r, neg_y, neg_dm_nbrs, neg_dm_nbrs_len, \
		neg_rg_nbrs, neg_rg_nbrs_len = self.get_negative_instance(in_batch = True)
		...
		pos_emb_r_sm_norm = tf.nn.embedding_lookup(self.rel_norm_embeddings, pos_emb_r)
		neg_emb_r_sm_norm = tf.nn.embedding_lookup(self.rel_norm_embeddings, neg_emb_r)
		pos_emb_h_sm = self._transfer(pos_emb_h_sm, pos_emb_r_sm_norm)
		pos_emb_t_sm = self._transfer(pos_emb_t_sm, pos_emb_r_sm_norm)
		neg_emb_h_sm = self._transfer(neg_emb_h_sm, neg_emb_r_sm_norm)
		neg_emb_t_sm = self._transfer(neg_emb_t_sm, neg_emb_r_sm_norm)

		pos_emb_h_type = tf.nn.embedding_lookup(self.ent_c_embeddings, pos_emb_h) #(bz,1,type_dim)
		...
		pos_rg_pred = tf.nn.sigmoid(tf.cast(tf.reduce_sum(tf.squeeze(pos_emb_t_type) * pos_rg_rexpt_hat, -1, keep_dims=True), dtype=tf.float32)) #(bz,1)
		neg_rg_pred = tf.nn.sigmoid(tf.cast(tf.reduce_sum(tf.squeeze(neg_emb_t_type) * neg_rg_rexpt_hat, -1, keep_dims=True), dtype=tf.float32))
		...
		pos_emb_score = tf.reduce_sum(self._calc(pos_emb_h_sm, pos_emb_t_sm, pos_emb_r_sm), -1, keep_dims=False) * config.k_rate + pos_axioms_score * config.s_rate
	...
	def predict_def(self):
		...
		predict_norm = tf.nn.embedding_lookup(self.rel_norm_embeddings, predict_emb_r)
		predict_emb_h_sm_e = self._transfer(predict_emb_h_sm_e, predict_norm)
		predict_emb_t_sm_e = self._transfer(predict_emb_t_sm_e, predict_norm)
